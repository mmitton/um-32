...
D        IF i > pwdcount THEN GOTO DD
DV       j = I
DX       IF CHECKPASS(username, words(i) + j) THEN GOTO DC
DXV      j = j + I
DXX      IF j < C THEN GOTO DX
DXXV     i = i + I
DXXX     GOTO D
DC       PRINT "found match!! for user " + username + CHR(X)
DCX      PRINT "password: " + words(i) + j + CHR(X)
DCV      END
DD       REM NO MATCH
DDV      PRINT "no expensive matches for user " + username + CHR(X)
DDX      END
STOP
